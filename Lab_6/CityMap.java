...
import mketour.observerpattern.ArtObserver;
import mketour.observerpattern.BusObserver;
import mketour.observerpattern.Observer;
import mketour.observerpattern.CarObserver;
...
public class CityMap extends Application {
    ...
    private void addEntities() {
        ...
        // Creates the sub-panes to be used by the challenges
        Pane msoePane = createSubPane(challengePane);
        Pane artPane = createSubPane(challengePane);
        Pane busPane = createSubPane(challengePane);

        // Creates the WordSearches for each challenge
        WordSearch msoeSearch = new WordSearch(("MSOE").toCharArray());
        WordSearch busSearch = new WordSearch(("BUS").toCharArray());

        // Creates a list of BusObservers for later reference
        List<BusObserver> busObservers = new ArrayList<>();
        ...
        for(int i = 0; i < NUM_TOURISTS; i++) {
            ...
            // Adds the respective observers for each car
            car.attach(new CarObserver(msoeSearch, car, msoePane));
            BusObserver busObserver = new BusObserver(busSearch, car, busPane);
            busObservers.add(busObserver);
            car.attach(busObserver);
        }
        ...
        for(int i = 0; i < NUM_BUSSES; i++) {
            ...
            // Adds the complete list of car observing BusObservers to each bus
        ...
        // Creates the single artObserver
        Observer artObserver = new ArtObserver(artPane);

        // Adds the ArtObservers to their respective subjects.
        for (Museum museum: museums){
            museum.attach(artObserver);
        }
    }

    /**
     * Method that is responsible for creating a subPane
     * from pane for use by the challenges.
     * @param pane the Pane that is being sub divided
     * @return the subPane of pane
     */
    private Pane createSubPane(Pane pane){
        Pane subPane = new VBox();
